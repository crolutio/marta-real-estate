...
### Requirement: Configurable hero content vertical placement

The hero video background SHALL support configurable vertical placement of its content: centered in the viewport, pinned to the top at all breakpoints, or pinned to the top on mobile and centered on `md` and up.

#### Scenario: Top-on-mobile placement

- **WHEN** a hero is configured for top-on-mobile placement
- **THEN** its content is pinned to the top of the section on mobile viewports
- **AND** its content is vertically centered on `md` and larger viewports
